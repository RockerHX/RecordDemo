...
class AudioKitController: UIViewController {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        let micDelay = AKDelay(micBooster)
        micDelay.time = 0.15
        micDelay.feedback = 0.6
        let micReverb = AKReverb(micDelay)
        micReverb.loadFactoryPreset(.largeChamber)
        let micMixture = AKDryWetMixer(micBooster, micReverb, balance: 0.1)

        let playerDelay = AKDelay(moogLadder)
        playerDelay.time = 0.15
        playerDelay.feedback = 0.6
        let playerReverb = AKReverb(playerDelay)
        playerReverb.loadFactoryPreset(.largeChamber)
        let playerMixture = AKDryWetMixer(moogLadder, playerReverb, balance: 0.1)

        mainMixer = AKMixer(playerMixture, micMixture)
    ...
    func setSliders(active: Bool) {
        ...
        frequencySlider.range = 10 ... 5_000
